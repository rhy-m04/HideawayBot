...
export function validateConfig(config) {
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!process.env.POSTGRES_HOST && !process.env.PGHOST && !process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
      errors.push("PostgreSQL host is required in production (POSTGRES_HOST or PGHOST environment variable)");
    }
    if (!process.env.POSTGRES_USER && !process.env.PGUSER && !process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
      errors.push("PostgreSQL user is required in production (POSTGRES_USER or PGUSER environment variable)");
    }
    if (!process.env.POSTGRES_PASSWORD && !process.env.PGPASSWORD && !process.env.DATABASE_URL && !process.env.POSTGRES_URL) {
      errors.push("PostgreSQL password is required in production (POSTGRES_PASSWORD or PGPASSWORD environment variable)");
    }
  }
